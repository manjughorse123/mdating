...
def send_notification(user_id,body,vals,data):
    ...
    try:
        ...
        msg= Message(
        notification=Notification(title=str(user_id.name), body=body, image=base_url+"/media/"+str(vals.profile_image)),data={"type":data,"user1":str(user_id.id),"user2":str(vals.id),"profile_image":str(vals.profile_image),"media":"media","base_url":str(base_url),"name":str(vals.name)}
            )
...
def send_notification1(user_id,title,body,data):
    ...
    try:
        ...
        device = CusztomFCMDevice.objects.filter(user=user_id).last()
        msg = Message(
        notification=Notification(title=str(title.name), body=body, 
        image=base_url+"/media/"+str(title.profile_image)),
        data={"type":data,"user1":str(user_id.id),"user2":str(title.id),"profile_image":str(title.profile_image),"media":"media","base_url":str(base_url),"name":str(title.name)})
        result = device.send_message(msg)
